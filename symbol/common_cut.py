...
def multi_layer_feature(body, from_layers, num_filters, strides, pads, min_filter=128):
    ...
    for k, params in enumerate(zip(from_layers, num_filters, strides, pads)):
        ...
        if from_layer.strip():
            ...
            conv_dw = Conv(layer, num_group=num_filters[k-1], num_filter=num_filters[k-1], kernel=(3, 3), pad=(p, p), stride=(s, s), name="feat_conv_dw_%d"%(k)) 
            conv_3 = Conv(conv_dw, num_filter=num_filter, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name="feat_conv_pw_%d"%(k)) 
