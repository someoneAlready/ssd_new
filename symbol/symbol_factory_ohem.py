...
import logging
from symbol import symbol_builder_ohem
import numpy as np
...
def get_config(network, data_shape, **kwargs):
    ...
    if network == 'vgg16_reduced':
        ...
        steps = []
        return locals()
    elif network == 'mobilenet_fit':
        from_layers = ['conv_12_relu', 'conv_14_relu', '', '', '', '', '']
        num_filters = [-1, -1, 512, 256, 256, 256, 256]
        strides = [-1, -1, 2, 2, 2, 2, 2]
        pads = [-1, -1, 1, 1, 1, 1, 1]
        sizes = get_scales(min_scale=0.15, max_scale=0.9, num_layers=len(from_layers))
        ratios = [[1./3, 1./2, 1./4]] * len(from_layers)
        normalizations = -1
        steps = []
        return locals()
    elif network == 'mobilenet_fitC':
        from_layers = ['conv_12_relu', 'conv_14_relu', '', '', '', '', '']
        num_filters = [-1, -1, 512, 256, 256, 256, 256]
        strides = [-1, -1, 2, 2, 2, 2, 2]
        pads = [-1, -1, 1, 1, 1, 1, 1]
        sizes = get_scales(min_scale=0.15, max_scale=0.9, num_layers=len(from_layers))
        ratios = [[3., 2., 4.]] * len(from_layers)
        normalizations = -1
        steps = []
        return locals()
    elif network == 'mobilenet_fitB':
        from_layers = ['conv_12_relu', 'conv_14_relu', '', '', '', '', '']
        num_filters = [-1, -1, 512, 256, 256, 256, 256]
        strides = [-1, -1, 2, 2, 2, 2, 2]
        pads = [-1, -1, 1, 1, 1, 1, 1]
        sizes = get_scales(min_scale=0.15, max_scale=0.9, num_layers=len(from_layers))
        ratios = [[1./3, 1./2, 1./4, 1./5, 1]] * len(from_layers)
        normalizations = -1
        steps = []
        return locals()
    elif network == 'mobilenet_cut':
        from_layers = ['conv_7_relu', '', '', '', '', '', '']
        num_filters = [-1, 512, 512, 256, 256, 256, 256]
...
def get_symbol_train(network, data_shape, **kwargs):
    ...
    data_shape : int
        input shape
    kwargs : dict
        see symbol_builder_ohem.get_symbol_train for more details
    """
    if network.startswith('legacy'):
        logging.warn('Using legacy model.')
        return symbol_builder_ohem.import_module(network).get_symbol_train(**kwargs)
    config = get_config(network, data_shape, **kwargs).copy()
    config.update(kwargs)
    return symbol_builder_ohem.get_symbol_train(**config)
...
def get_symbol(network, data_shape, **kwargs):
    ...
    data_shape : int
        input shape
    kwargs : dict
        see symbol_builder_ohem.get_symbol for more details
    """
    if network.startswith('legacy'):
        logging.warn('Using legacy model.')
        return symbol_builder_ohem.import_module(network).get_symbol(**kwargs)
    config = get_config(network, data_shape, **kwargs).copy()
    config.update(kwargs)
    return symbol_builder_ohem.get_symbol(**config)
