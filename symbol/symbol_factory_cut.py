...
import logging
from symbol import symbol_builder_cut
import numpy as np
...
def get_symbol_train(network, data_shape, **kwargs):
    ...
    data_shape : int
        input shape
    kwargs : dict
        see symbol_builder_cut.get_symbol_train for more details
    """
    if network.startswith('legacy'):
        logging.warn('Using legacy model.')
        return symbol_builder_cut.import_module(network).get_symbol_train(**kwargs)
    config = get_config(network, data_shape, **kwargs).copy()
    config.update(kwargs)
    return symbol_builder_cut.get_symbol_train(**config)
...
def get_symbol(network, data_shape, **kwargs):
    ...
    data_shape : int
        input shape
    kwargs : dict
        see symbol_builder_cut.get_symbol for more details
    """
    if network.startswith('legacy'):
        logging.warn('Using legacy model.')
        return symbol_builder_cut.import_module(network).get_symbol(**kwargs)
    config = get_config(network, data_shape, **kwargs).copy()
    config.update(kwargs)
    return symbol_builder_cut.get_symbol(**config)
