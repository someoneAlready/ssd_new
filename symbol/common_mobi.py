...
def multi_layer_feature_mobi(body, from_layers, num_filters, strides, pads, min_filter=128):
    """Wrapper function to extract features from base network, attaching extra
    layers and SSD specific layers
    ...
    for k, params in enumerate(zip(from_layers, num_filters, strides, pads)):
        ...
        if from_layer.strip():
            ...
            assert len(layers) > 0
            assert num_filter > 0
            num_filter_prev = num_filters[k-1]
            layer = layers[-1]

            conv_dw = Conv(layer, num_group=num_filter_prev, num_filter=num_filter_prev, kernel=(3, 3), pad=(1, 1), stride=(2, 2), name='multi_feat_%d_conv_1x1' % (k))
            conv_pw = Conv(conv_dw, num_filter=num_filter, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name='multi_feat_%d_conv_3x3' % (k))
